...
| 메서드 | 경로 |
| --- | --- |
| `POST` | `/auth/forgot-password` — 가입 여부와 무관하게 항상 같은 응답 |
| `POST` | `/auth/reset-password` — 1시간 유효, 한 번만 사용 |

토큰은 `password_reset_tokens`에 **해시만** 저장합니다. 관련 환경변수는 둘입니다.

| 변수 | 없을 때 |
| --- | --- |
| `SMTP_URL` | 메일을 보내지 않고 링크를 로그에 남깁니다 |
| `WEB_ORIGIN` | 요청 헤더의 Origin으로 내려갑니다 (운영에서는 반드시 설정) |
